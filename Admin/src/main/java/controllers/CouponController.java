...
public class CouponController {
    ...
    public ModelAndView index() throws IOException {
        return new ModelAndView("coupon").addObject("coupons", restHelper.getAll()).addObject("restaurants",restHelperrestaurant.getAll());
    }
    ...
    @RequestMapping(value = "/coupon/{id}", method = RequestMethod.GET, produces = "application/json;charset=UTF-8")
    @ResponseBody
    public String getCoupon(@PathVariable("id") String id) throws IOException {
        String s = restHelper.getCoupon(id);
        return s;
    }
    ...
    @RequestMapping(value = "/coupon/updateCoupon", method = RequestMethod.POST)
    public ModelAndView update(MultipartContainer multipartContainer, Coupon coupon, HttpServletRequest request, HttpServletResponse responseServlet) throws IOException, ServletException {
        MultipartFile[] multipartFile = multipartContainer.getMultipartFile();
        String path = "./";
        FileDataBodyPart filePart;
        Client client = ClientBuilder.newBuilder().register(MultiPartFeature.class
