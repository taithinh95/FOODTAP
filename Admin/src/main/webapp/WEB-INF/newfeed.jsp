            ...
            <div class="modal-body">
                <form>
                    ...
                    <div class="text-center" style="margin-top: 10px;">
                        <button  class="btn btn-danger" type="submit" data-dismiss="modal">Thêm</button>
                    </div>
                </form>
            </div>
        </div>
                ...
                <li class="nav-item dropdown no-arrow">
                    ...
                    <div class="dropdown-menu dropdown-menu-right shadow animated--grow-in" aria-labelledby="userDropdown">
                        <a class="dropdown-item" href="#">
                            <i class="fas fa-user fa-sm fa-fw mr-2 text-gray-400"></i> Chuyển về trang người dùng
                        </a>
                            ...
                            <div class="row">
                                <div class="col-sm-12">
                                    <table class="table table-bordered table-hover dataTable" id="dataTable" width="100%" cellspacing="0" role="grid" aria-describedby="dataTable_info" style="width: 100%;">
                                        <thead>
                                            <tr role="row">
                                                <th class="sorting_asc" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 150px;">Người đăng</th>
                                                <th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1"  style="width: 150px;">Ảnh</th>
                                                <th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1"  style="width: 50px;">Lượt thích</th>
                                                <th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 200px;">Nội dung</th>
                                        ...
                                        <tbody>
                                            <c:forEach var="item" items="${newfeeds}">
                                                <tr role="row" class="odd odd-coupon">
                                                    <c:choose>
                                                        <c:when test="${item.user.fullname!=null}">
                                                            <td class="sorting_1">${item.user.fullname}</td>
                                                        </c:when>
                                                        <c:otherwise>
                                                            <td class="sorting_1">${item.restaurant.name}</td>
                                                        </c:otherwise>
                                                    </c:choose>
                                                    <td class="sorting"><img src="http://localhost:9032/public/image/${item.images[0]}" width="200px" /></td>
                                                    <c:set var="countLike" value="${0}"/>
                                                    <c:forEach var="c" items="${item.likes}">
                                                    ...
                                                    <td class="d-flex">
                                                        <c:choose>
                                                            <c:when test="${item.active == 'true'}">
                                                                <button type="button" class="btn btn-danger form-control" idValue="${item._id}" id="hiddenNewfeed">Ẩn</button>
                                                            </c:when>
                                                            <c:otherwise>
                                                                <button type="button" class="btn btn-success form-control" idValue="${item._id}" id="showNewfeed">Hiện</button>
                                                            </c:otherwise>
                                                        </c:choose>
                                                    </td>
                                                </tr>
